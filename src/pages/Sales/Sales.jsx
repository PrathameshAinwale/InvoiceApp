...
const Sales = () => {
  ...
  const monthlyData = MONTHS.map((month, i) => {
    ...
    return {
      month,
      revenue: parseFloat(
        monthSales.reduce((sum, s) => sum + s.finalAmount, 0).toFixed(2),
      ),
  ...
  sales.forEach((s) => {
    ...
    if (s.status === "completed") {
      yearlyMap[year].revenue = parseFloat(
        (yearlyMap[year].revenue + s.finalAmount).toFixed(2),
      );
  ...
  const statusData = [
    ...
    {
      name: "Refunded",
      value: sales.filter((s) => s.status === "refunded").length,
    },
    {
      name: "Cancelled",
  ...
  sales.forEach((s) => {
    ...
    if (s.status === "completed") {
      productMap[s.product].revenue = parseFloat(
        (productMap[s.product].revenue + s.finalAmount).toFixed(2),
      );
      productMap[s.product].count += 1;
      productMap[s.product].months.add(new Date(s.date).getMonth()); // ← track unique months
    }
  });
  const productData = Object.values(productMap)
    ...
    .map((p) => ({
      ...p,
      monthCount: p.months.size, // ← number of months with sales
    ...
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          ...
          <p style={{ color: "#667eea", margin: "2px 0" }}>
            Revenue: ${payload[0]?.value?.toLocaleString()}
          </p>
          <p style={{ color: "#aaa", margin: "2px 0" }}>Sales: {p?.count}</p>
        </div>
      );
    }
    return null;
        ...
        <div className="chart-card">
          ...
          <div className="chart-meta">
            <div className="chart-meta-row">
              {activeChart === "monthly" && (
                <p className="chart-title">Revenue by Month</p>
              )}
              {activeChart === "yearly" && (
                <p className="chart-title">Yearly Revenue Growth</p>
              )}
              {activeChart === "status" && (
                <p className="chart-title">Sales by Status</p>
              )}
              {activeChart === "products" && (
                <p className="chart-title">Revenue by Product</p>
              )}
            ...
            {activeChart === "monthly" && (
              <p className="chart-sub">Completed sales this year</p>
            )}
            {activeChart === "yearly" && (
              <p className="chart-sub">
                {yearlyData.length} year{yearlyData.length !== 1 ? "s" : ""} of
                data
              </p>
            )}
            {activeChart === "status" && (
              <p className="chart-sub">{sales.length} total sales</p>
            )}
            {activeChart === "products" && (
              <p className="chart-sub">{productData.length} products sold</p>
            )}
          </div>
          ...
          {activeChart === "products" && (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart
                data={productData}
                margin={{ top: 24, right: 10, left: 10, bottom: 50 }}
              >
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis
                  dataKey="shortName"
                ...
                <YAxis
                  ...
                  domain={[0, 12]}
                  ticks={[0, 3, 6, 9, 12]}
                  tickFormatter={(val) => (val === 0 ? "0" : `${val}m`)} // ← 3m, 6m, 9m, 12m
                />
                <Tooltip
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p style={{ color: "#667eea", margin: "2px 0" }}>
                            Months active: {p?.monthCount}m
                          </p>
                          <p style={{ color: "#aaa", margin: "2px 0" }}>
                ...
                />
                {/* Bar shows monthCount, label on top shows revenue amount */}
